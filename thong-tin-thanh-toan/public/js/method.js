...
function SetTitleAndIcon(title, icon) {
	...
	link.href = `../thong-tin-thanh-toan/public/images/icons/${icon}`;
...
function RenderProfileHeader(username, avatar, skills = [], blue = true) {
	...
	htmlProfile += `
        <img alt="Avatar" 
            src="../thong-tin-thanh-toan/public/images/avatars/${avatar}" 
...
function RenderBanks(banks = []) {
	...
	banks.forEach((bank) => {
		let classShowModal =
			bank.hasOwnProperty("fastTransfer") && bank.fastTransfer
				? "btn-show-modal"
				: "";
		htmlBanks += `
                <div class="bank">
                    <img src="../thong-tin-thanh-toan/public/images/banks/${bank.logo}" class="bank-logo">
                    <div class="bank-info">
